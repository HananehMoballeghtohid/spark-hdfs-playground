...
docker-compose -f docker-compose.yml up -d zookeeper1 zookeeper2 zookeeper3
...
docker-compose -f docker-compose.yml up -d journalnode1 journalnode2 journalnode3
...
docker-compose -f docker-compose.yml run --rm namenode1 hdfs namenode -format -force

# Start the first NameNode
echo "Starting NameNode1..."
docker-compose -f docker-compose.yml up -d namenode1
...
docker-compose -f docker-compose.yml run --rm namenode2 hdfs namenode -bootstrapStandby

# Format ZKFC on both NameNodes
echo "Formatting ZKFC..."
docker-compose -f docker-compose.yml exec namenode1 hdfs zkfc -formatZK -force

# Start the second NameNode
echo "Starting NameNode2..."
docker-compose -f docker-compose.yml up -d namenode2

# Start ZKFC on both namenodes to enable automatic failover
echo "Starting ZKFC daemons..."
docker-compose -f docker-compose.yml exec -d namenode1 hdfs zkfc
docker-compose -f docker-compose.yml exec -d namenode2 hdfs zkfc
...
docker-compose -f docker-compose.yml up -d datanode1 datanode2 datanode3
docker-compose -f docker-compose.yml up -d resourcemanager1 resourcemanager2
docker-compose -f docker-compose.yml up -d nodemanager1 nodemanager2 nodemanager3
...
ACTIVE_NN=""
if docker-compose -f docker-compose.yml exec -T namenode1 hdfs haadmin -getServiceState nn1 2>/dev/null | grep -q "active"; then
    ACTIVE_NN="namenode1"
    echo "NameNode1 is active"
elif docker-compose -f docker-compose.yml exec -T namenode2 hdfs haadmin -getServiceState nn2 2>/dev/null | grep -q "active"; then
    ACTIVE_NN="namenode2"
...
docker-compose -f docker-compose.yml exec -T $ACTIVE_NN hdfs dfs -mkdir -p /spark-logs
docker-compose -f docker-compose.yml exec -T $ACTIVE_NN hdfs dfs -mkdir -p /spark-warehouse
docker-compose -f docker-compose.yml exec -T $ACTIVE_NN hdfs dfs -mkdir -p /spark/jars
docker-compose -f docker-compose.yml exec -T $ACTIVE_NN hdfs dfs -chmod -R 777 /spark-logs
docker-compose -f docker-compose.yml exec -T $ACTIVE_NN hdfs dfs -chmod -R 777 /spark-warehouse
docker-compose -f docker-compose.yml exec -T $ACTIVE_NN hdfs dfs -chmod -R 777 /spark

# Start Spark services
echo "Starting Spark services..."
docker-compose -f docker-compose.yml up -d spark-master
...
# Start Spark workers
docker-compose -f docker-compose.yml up -d spark-worker1 spark-worker2 spark-worker3
...
docker-compose -f docker-compose.yml up -d spark-history-server jupyter-spark
...
echo "  docker-compose -f docker-compose.yml exec namenode1 hdfs haadmin -getServiceState nn1"
echo "  docker-compose -f docker-compose.yml exec namenode1 hdfs haadmin -getServiceState nn2"
